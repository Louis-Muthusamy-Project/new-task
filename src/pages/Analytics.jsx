...
import { Typography, Row, Col, Card, Select, Button, Table, Tag } from 'antd';
import { motion } from 'framer-motion';
import { Download, Calendar as CalendarIcon, Filter, RefreshCw, BarChart2, PieChart as PieChartIcon } from 'lucide-react';
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible">
      <motion.div variants={itemVariants} style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <Title level={2} style={{ margin: '4px 0 0 0', fontWeight: 800 }}>Analytics</Title>
          <Text type="secondary">Unified performance data across all channels and clients.</Text>
        </div>
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          ...
          <Button type="primary" icon={<Download size={16} />} style={{ borderRadius: 8, height: 40, background: 'var(--accent-secondary)', color: '#fff', border: 'none', boxShadow: 'var(--shadow-md)', fontWeight: 600 }}>Export</Button>
        </div>
      </motion.div>
      ...
      <motion.div variants={itemVariants} style={{ display: 'flex', gap: 32, borderBottom: '1px solid var(--border-color)', marginBottom: 32, overflowX: 'auto', paddingBottom: 2 }}>
        <div style={{ paddingBottom: 16, borderBottom: '3px solid var(--accent-secondary)', fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', whiteSpace: 'nowrap' }}><BarChart2 size={18} color="var(--accent-secondary)"/> Analytics</div>
        <div style={{ paddingBottom: 16, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 10, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'color 0.2s' }}><PieChartIcon size={18} /> Attribution</div>
      </motion.div>
