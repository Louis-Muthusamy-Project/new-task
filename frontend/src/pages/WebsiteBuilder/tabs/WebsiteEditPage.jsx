...
const WebsiteEditPage = ({ website: initialWebsite, onBack, onChange, justCreated = true }) => {
  ...
  const handlePreview = async (pageSlug) => {
    ...
    try {
      ...
      // If a chat widget is assigned to this website, fetch its full config
      // so the floating launcher + details box can be rendered in the preview.
      let chatWidget = null;
      const assignedWidgetId = previewData.website?.chatWidgetId;
      if (assignedWidgetId) {
        try {
          chatWidget = await websiteWizardApi.getChatWidget(assignedWidgetId);
        } catch (widgetErr) {
          console.warn("[WebsiteEditPage] failed to load assigned chat widget:", widgetErr);
        }
      }
  ...
  // ─── Chat widgets (for the "Chat widget" dropdown) ───────────────────────
  const [chatWidgets, setChatWidgets] = useState([]);
  const [chatWidgetsLoading, setChatWidgetsLoading] = useState(false);
  ...
  const fetchChatWidgets = async () => {
    try {
      setChatWidgetsLoading(true);
      const data = await websiteWizardApi.listChatWidgets();
      const list = Array.isArray(data) ? data : [];
      console.log("[WebsiteEditPage] chat widgets fetched:", list.length, list);
      setChatWidgets(list);
    } catch (err) {
      console.error("[WebsiteEditPage] fetchChatWidgets failed:", err);
      message.error("Could not load chat widgets — check that the backend is running.");
    } finally {
      setChatWidgetsLoading(false);
    }
  };
  ...
  const handleSaveChatWidget = async () => {
    if (!websiteDbId) {
      message.error("Save the website first before assigning a chat widget.");
      return;
    }
    try {
      setSavingChatWidget(true);
      await websiteWizardApi.updateWebsite(websiteDbId, {
        chatWidgetId: website.chatWidgetId || null,
      });
      message.success("Chat widget saved.");
    } catch (err) {
      console.error("[WebsiteEditPage] save chat widget failed:", err);
      message.error("Failed to save chat widget.");
    } finally {
        ...
        <Col xs={24} lg={11}>
          ...
          >
            ...
            <div style={{ marginTop: 24, paddingTop: 20, borderTop: "1px solid var(--border-color)" }}>
              ...
              <Button
                block
                loading={savingChatWidget}
                disabled={!websiteDbId}
                onClick={handleSaveChatWidget}
                style={{ height: 44, borderRadius: 10, background: "var(--accent-info)", color: "#fff", border: "none", fontWeight: 700 }}
              >
                Save chat widget
              </Button>
